...
class Main {
 ...
 public static void main(String[] args)throws IOException{
  ...
  void init() {
    ...
    sql = "UPDATE students SET room = 122 WHERE studentid = 'STUDENT1001' AND period = 3;";

    db.runSQL(sql);

    // Challenge 2:
    // Remove course “ZQCTEDA” for student with id STUDENT1200 
    sql = "Delete From students WHERE studentid = 'STUDENT1200' AND course = 'ZQCTEDA';";

    db.runSQL(sql);
    
    // Challenge 3:
    // Change the room number of teacher1 DOYLE for periods 4 and 5 to room 213.
    sql = "UPDATE students SET room = 213 WHERE teacher1 = 'DOYLE' AND periods = 4 AND periods = 5;";

    db.runSQL(sql);
    
    // Challenge 4:
    // Mr. ROFFLER will be replacing Mr. ARCHETTI for course “MQF44QGF” section 1. Update all records to reflect this change.
    sql = "UPDATE students SET teacher1 = 'ROFFLER' WHERE teacher1 = 'ARCHETTI' AND course = 'MQF44QGF' AND section = 1;";

    db.runSQL(sql);
    
    // Challenge 5:
    // Change the grade for STUDENT999 to 11 and the students offcclass to a junior status.
    sql = "Update students SET grade = '11', offclass = '3C8' WHERE studentid='STUDENT999';";

    db.runSQL(sql);
    
    // Challenge 6:
    // Add a course MKUFTC6 with a section 1. CASTRO R will be teaching this course period 9 in room 322 for student1231.
    sql = "INSERT INTO students (course, section, teacher1, period, room, studentid) VALUES('MKUFTC6', 1, 'CASTRO R', 9, 322, 'STUDENT1231');";
